...
# Comprehensive mapping based on the ASSET_MAP in index.html
ASSET_MAP = {
...
# Add relative path versions and variations
EXTRAS = {
...
def fix_file(file_path, is_in_assets=False):
    ...
    all_keys = sorted(list(ASSET_MAP.keys()) + list(EXTRAS.keys()), key=len, reverse=True)
    
    combined_map = {**ASSET_MAP, **EXTRAS}
    
    for key in all_keys:
        val = combined_map[key]
        
        # If the file is in assets, its relative path to other assets is just the filename
        # If it's to index.html, it's ../index.html
        if is_in_assets:
            if val == "index.html":
                target = "../index.html"
            else:
            ...
            # If it's index.html, targets are in assets/ (except index.html itself)
            if val == "index.html":
                target = "index.html"
            else:
                target = "assets/" + val
        
        # Replace absolute and root-relative
        new_content = new_content.replace('href="' + key + '"', 'href="' + target + '"')
        new_content = new_content.replace('src="' + key + '"', 'src="' + target + '"')
        # Variations with ?
        new_content = new_content.replace('href="' + key + '?"', 'href="' + target + '"')
        new_content = new_content.replace('src="' + key + '?"', 'src="' + target + '"')
        # Variations with &amp;
        new_content = new_content.replace(key.replace('&', '&amp;'), target)
...
def main():
    ...
    # Fix index.html
    fix_file(os.path.join(root, 'index.html'), False)
    
    # Fix assets
    assets_dir = os.path.join(root, 'assets')
    for filename in os.listdir(assets_dir):
        if filename.endswith('.html') or filename.endswith('.php'):
            fix_file(os.path.join(assets_dir, filename), True)
